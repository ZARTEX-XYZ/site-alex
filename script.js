...
// Books comme boutons cliquables
const books = document.querySelectorAll('.book');
books.forEach(book => {
  book.addEventListener('click', () => {
    const link = book.querySelector('.book-btn');
    if (link && link.href) {
...
if (cookieBanner && cookieAccept && cookieReject) {
  ...
  cookieAccept.addEventListener('click', () => {
    ...
    // Ici, vous pouvez activer des scripts nécessitant le consentement
  });

  cookieReject.addEventListener('click', () => {
    setCookieConsent('rejected');
    cookieBanner.style.display = 'none';
    // Ici, vous pouvez désactiver ou ne pas activer certains scripts
  });
}
